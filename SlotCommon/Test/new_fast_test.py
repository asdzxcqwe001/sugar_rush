...
def spin(calculator: SlotCommon.slot_calculator.DefaultSlotCalculator,
    ...
    result = calculator.spin(bet_value=bet_value, bet_lines=bet_lines, game_state=game_state, gameInfo=info, dev_mode=0,
...
def next_fever(calculator: SlotCommon.slot_calculator.DefaultSlotCalculator,
    ...
    if calculator.is_in_game_jackpot_exist():
        try:
            for i in range(20):
                result = calculator.next_fever(client_action, game_state, gameInfo=info, dev_mode=dev_mode)
                if result.has_error:
...
def _LoadGameList():
    # for送審，寫死不走動態
    return '', ['LionDance'], ['LionDance.Game.Slot']
    #
    # def get_games_dir(package):
    #     games_path = importlib.resources.files(package)
    #     return [entry.name for entry in games_path.iterdir() if entry.is_dir()]
    #
    # def check_game_exist(package, game_name):
    #     # 檢查 calculator / Chance / Check / Setting / Info 是否存在
    #     check_list = ['', 'Chance', 'Check', 'Setting', 'Info']
    #     game_package = package + '.' + game_name
    #     game_path = importlib.resources.files(game_package)
    #
    #     for check in check_list:
    #         file_name = f"{game_name}{check}.py"
    #         if not game_path.joinpath(file_name).is_file():
    #             return False
    #     return True
    #
    # package_path = 'LionDance.Game.Slot'
    # name_list = []
    # path_list = []
    # dirs = get_games_dir(package_path)
    # for folder in dirs:
    #     if check_game_exist(package_path, folder):
...
def _CreateLogger(game_id, bet_value, probability_ver=''):
    import time
    ...
    if not os.path.isdir(path):
        os.mkdir(path)
    logger = DoubleOutput(
        path + "_".join(["/fast_test", game_id, probability_ver, str(bet_value), ts]) + ".txt",
        path + "_".join([path, "/fast_test", game_id, probability_ver, str(bet_value), ts, "ERR.txt"])
    )
...
if __name__ == "__main__":
    ...
    info.update(setting)
    logger = _CreateLogger(game_id, bet)
